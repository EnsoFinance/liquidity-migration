    ...
    try {
      while (migrations.length > 0) {
        ...
        while (users.length > 0) {
          ...
          let tx;
          try {
            tx = await migrationCoordinator.migrateLP(subset, migrationData["lp"], migrationData["adapter"], {
...
const waitForLowGas = async (signer: any) => {
  return new Promise<any>(async resolve => {
    ...
    const [block, feeData] = await Promise.all([provider.getBlock(blockNumber), signer.getFeeData()]);
