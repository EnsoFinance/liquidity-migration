...
describe("Migrator tests: ", function () {
  ...
  it("Should deploy new liquidity migration contract", async function () {
    ...
    const mockControllerImplementation = await MockController.connect(signers.admin).deploy(
      ...
      mockLiquidityMigration.address,
      signers.admin.address,
    );
  ...
  it("Should batch migrate", async function () {
    ...
    const users = stakers.filter((account: string, index: number) => stakers.indexOf(account) === index).slice(0, 150);
    ...
    tx = await mockLiquidityMigration.connect(signers.admin).batchSetStake(users, lps, stakes);
    ...
    tx = await mockLiquidityMigration.connect(signers.admin).batchMigrate(users, dpiPoolAddress, dpiStrategy.address);
    receipt = await tx.wait();
