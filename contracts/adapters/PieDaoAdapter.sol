...
contract PieDaoAdapter is AbstractAdapter {
    ...
    function outputTokens(address _lp) public view override returns (address[] memory outputs) {
    ...
    {
        calls = new Call[](1);
        calls[0] = Call(payable(_lp), abi.encodeWithSelector(IPieDaoPool(_lp).exitPool.selector, _amount));
    }
}
