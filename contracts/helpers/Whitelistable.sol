...
abstract contract Whitelistable is Ownable {
     ...
     */
    function add(address _token) public onlyOwner {
        _add(_token);
    }
     ...
     */
    function addBatch(address[] memory _tokens) public onlyOwner {
     ...
     * @param _token pool address
     */
    function remove(address _token) public onlyOwner {
        _remove(_token);
    }
     ...
     * @param _tokens[] array of pool address
     */
    function removeBatch(address[] memory _tokens) public onlyOwner {
        for (uint256 i = 0; i < _tokens.length; i++) {
            _remove(_tokens[i]);
        }
    }

    function _add(address _token) internal {
        whitelisted[_token] = true;
        emit Added(_token);
    }

    function _remove(address _token) internal {
