...
import { MockFundHolder__factory } from "../typechain";
...
describe("Timelocked Test", function () {
  beforeEach(async function () {
    ...
    console.log(`Approximate Deployment blocktime is ${this.blocktimestamp.toString()}`);
  ...
  it("only the Owner of the Contract is able update Modify blocktime", async function () {
    ...
    await expect(
      this.mockFundHolder.connect(this.signers.otherAccount).updateUnlock(newBlockTimeForModification),
  ...
  it("Owner of the Contract is not able update Modify if the Modify timeperiod has lapsed", async function () {
    ...
    await expect(
      this.mockFundHolder.connect(this.signers.default).updateUnlock(newBlockTimeForModification),
    ).to.be.revertedWith("Timelock#onlyModify: cannot modify");
  });
  ...
  it("depositer is not able to withdraw before the unlocked time", async function () {
    ...
    await expect(
      this.mockFundHolder.connect(this.signers.otherAccount).withdraw(ethers.utils.parseEther("1.0")),
    ).to.be.revertedWith("Timelock#onlyUnlocked: not unlocked");
  });
