...
describe("V2 Migration: ", function () {
  ...
  it("Should fail to stake on V1", async function () {
    ...
    await expect(liquidityMigration.connect(user).stake(lpPool.address, 1, indexCoopAdapterAddress)).to.be.revertedWith(
      "Claimable#onlyState: not registered adapter",
    );
    await expect(
      liquidityMigration.connect(user).stake(lpPool.address, 1, migrationAdapter.address),
  ...
  it("Should withdraw lp tokens via coordinator", async function () {
    ...
    await migrationCoordinator.connect(user).withdraw(dpiPool.address);
  ...
  it("Should fail withdraw lp tokens via coordinator: not staking", async function () {
    await expect(migrationCoordinator.connect(signers.admin).withdraw(dpiPool.address)).to.be.revertedWith(
      "LiquidityMigration#_refund: no stake",
    );
  });

  it("Should refund lp tokens via coordinator", async function () {
    const balanceBefore = await dpiPool.balanceOf(dpiStakers[0]);
    await migrationCoordinator.connect(signers.admin).refund(dpiStakers[0], dpiPool.address);
  ...
  it("Should fail refund lp tokens via coordinator: not owner", async function () {
    await expect(
      migrationCoordinator.connect(signers.default).refund(dpiStakers[0], dpiPool.address),
  ...
  it("Should migrate to new LiquidityMigration contract", async function () {
    ...
    const tx = await migrationCoordinator
      .connect(signers.admin)
      .migrateLP(users, dpiPoolAddress, indexCoopAdapterAddress);
    const receipt = await tx.wait();
  ...
  it("Should withdraw lp tokens via LMV2", async function () {
    ...
    const balanceBefore = await dpiPool.balanceOf(user.address);
    await liquidityMigrationV2.connect(user).withdraw(dpiPool.address);
    const balanceAfter = await dpiPool.balanceOf(user.address);
    expect(balanceAfter.gt(balanceBefore)).to.equal(true);
    // Remove user from array
    users.shift();
  ...
  it("Should fail withdraw lp tokens via LMV2: not staking", async function () {
    await expect(liquidityMigrationV2.connect(signers.admin).withdraw(dpiPool.address)).to.be.revertedWith("No stake");
  });

  it("Should refund lp tokens via LMV2", async function () {
    const balanceBefore = await dpiPool.balanceOf(users[0]);
    await liquidityMigrationV2.connect(signers.admin).refund(users[0], dpiPool.address);
  ...
  it("Should fail refund lp tokens via LMV2: not owner", async function () {
    await expect(liquidityMigrationV2.connect(signers.default).refund(users[0], dpiPool.address)).to.be.revertedWith(
      "Ownable: caller is not the owner",
    );
  });

  it("Should fail to remove adapter", async function () {
    await expect(migrationCoordinator.connect(signers.admin).removeAdapter(indexCoopAdapterAddress)).to.be.revertedWith(
      "LiquidityMigration#updateAdapter: does not exist",
    );
  });

  it("Should fail to add adapter", async function () {
    await expect(migrationCoordinator.connect(signers.default).addAdapter(indexCoopAdapterAddress)).to.be.revertedWith(
      "Ownable: caller is not the owner",
    );
  });

  it("Should add adapter", async function () {
    await migrationCoordinator.connect(signers.admin).addAdapter(indexCoopAdapterAddress);
  ...
  it("Should fail to remove adapter", async function () {
    await expect(
      migrationCoordinator.connect(signers.default).removeAdapter(indexCoopAdapterAddress),
    ).to.be.revertedWith("Ownable: caller is not the owner");
  });

  it("Should remove adapter", async function () {
    await migrationCoordinator.connect(signers.admin).removeAdapter(indexCoopAdapterAddress);
  ...
  it("Should fail to update migrator: not owner", async function () {
    await expect(
      migrationCoordinator.connect(signers.default).updateMigrator(signers.default.address),
    ).to.be.revertedWith("Ownable: caller is not the owner");
  });

  it("Should fail to update migrator: already exists", async function () {
    await expect(migrationCoordinator.connect(signers.admin).updateMigrator(signers.admin.address)).to.be.revertedWith(
      "Already exists",
    );
  });

  it("Should update migrator", async function () {
    await migrationCoordinator.connect(signers.admin).updateMigrator(signers.default.address);
  ...
  it("Should fail claim strategy tokens: not claimable", async function () {
    ...
    await expect(liquidityMigrationV2.connect(user).claim(dpiPool.address)).to.be.revertedWith("Not yet migrated");
  });
  ...
  it("Migrate LP liquidity", async function () {
    ...
    const tx = await liquidityMigrationV2.connect(signers.admin).migrateAll(dpiPoolAddress, indexCoopAdapterAddress);
    const receipt = await tx.wait();
  ...
  it("Should fail refund lp tokens via LMV2: not refundable", async function () {
    await expect(liquidityMigrationV2.connect(signers.admin).refund(users[0], dpiPool.address)).to.be.revertedWith(
      "Not refundable",
    );
  });
  ...
  it("Should fail to set strategy", async function () {
    ...
    await expect(
      liquidityMigrationV2.connect(signers.admin).setStrategy(dpiPool.address, dpiStrategy.address),
  ...
  it("Should fail transfer LMV1 ownership: not owner", async function () {
    await expect(
      migrationCoordinator.connect(signers.default).transferLiquidityMigrationOwnership(signers.default.address),
    ).to.be.revertedWith("Ownable: caller is not the owner");
  });

  it("Should transfer LMV1 ownership", async function () {
    await migrationCoordinator.connect(signers.admin).transferLiquidityMigrationOwnership(signers.admin.address);
  ...
  it("Should fail to emergency transfer: not paused", async function () {
    await expect(liquidityMigrationV2.connect(signers.admin).emergencyMigrate(dpiStrategy.address)).to.be.revertedWith(
      "Not paused",
    );
  });

  it("Should fail to pause: not owner", async function () {
    await expect(liquidityMigrationV2.connect(signers.default).pause()).to.be.revertedWith(
      "Ownable: caller is not the owner",
    );
  });
  ...
  it("Should fail to emergency transfer: not owner", async function () {
    await expect(
      liquidityMigrationV2.connect(signers.default).emergencyMigrate(dpiStrategy.address),
    ).to.be.revertedWith("Ownable: caller is not the owner");
  });

  it("Should fail to emergency transfer: no receiver", async function () {
    await expect(liquidityMigrationV2.connect(signers.admin).emergencyMigrate(dpiStrategy.address)).to.be.revertedWith(
      "Emergency receiver not set",
    );
  });
  ...
  it("Should fail to emergency transfer: no balance", async function () {
    await expect(liquidityMigrationV2.connect(signers.admin).emergencyMigrate(dpiPool.address)).to.be.revertedWith(
      "No balance",
    );
  });
