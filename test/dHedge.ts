...
describe("dHedge: Unit tests", function () {
  ...
  before(async function () {
    ...
    await chainLinkRegistries
      .connect(this.signers.admin)
      .addOracle(SUSD, WETH, "0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419", true); //sUSD
    await chainLinkRegistries
      .connect(this.signers.admin)
      .addOracle(sETH, SUSD, "0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419", false); //sETH
    await chainLinkRegistries
      .connect(this.signers.admin)
      .addOracle(sAAVE, WETH, "0x6df09e975c830ecae5bd4ed9d90f3a95a4f88012", false); //sAAVE
    await chainLinkRegistries
      .connect(this.signers.admin)
      .addOracle(sBTC, WETH, "0xdeb288f737066589598e9214e782fa5a8ed689e8", false); //sBTC
    await chainLinkRegistries
      .connect(this.signers.admin)
      .addOracle(sDOT, SUSD, "0x1c07afb8e2b827c5a4739c6d59ae3a5035f28734", false); //sDOT
    await chainLinkRegistries
      .connect(this.signers.admin)
      .addOracle(sADA, SUSD, "0xae48c91df1fe419994ffda27da09d5ac69c30f55", false); //sADA

    // setup synth estimator
    await this.enso.platform.strategyFactory
      .connect(this.signers.admin)
      .addItemToRegistry(ITEM_CATEGORY.SYNTH, ESTIMATOR_CATEGORY.CHAINLINK_ORACLE, sETH);
    await this.enso.platform.strategyFactory
      .connect(this.signers.admin)
      .addItemToRegistry(ITEM_CATEGORY.SYNTH, ESTIMATOR_CATEGORY.CHAINLINK_ORACLE, sAAVE);
    await this.enso.platform.strategyFactory
      .connect(this.signers.admin)
      .addItemToRegistry(ITEM_CATEGORY.SYNTH, ESTIMATOR_CATEGORY.CHAINLINK_ORACLE, sBTC);
    await this.enso.platform.strategyFactory
      .connect(this.signers.admin)
      .addItemToRegistry(ITEM_CATEGORY.SYNTH, ESTIMATOR_CATEGORY.CHAINLINK_ORACLE, sDOT);
    await this.enso.platform.strategyFactory
      .connect(this.signers.admin)
      .addItemToRegistry(ITEM_CATEGORY.SYNTH, ESTIMATOR_CATEGORY.CHAINLINK_ORACLE, sADA);
  });
  ...
  it("Token holder should be able to withdraw from pool", async function () {
    ...
    for (let i = 0; i < this.DHedgeEnv.holders.length; i++) {
      ...
      expect(await this.erc20.balanceOf(await this.DHedgeEnv.holders[i].getAddress())).to.gt(BigNumber.from(0));
    }
    ...
    const tx = await this.DHedgeEnv.pool.connect(this.DHedgeEnv.holders[0]).withdraw(previoustokenBalance);
    await tx.wait();
    const posttokenBalance = await this.erc20.balanceOf(await this.DHedgeEnv.holders[0].getAddress());
    expect(posttokenBalance.isZero()).to.be.true;
  });

  it("Token holder should be able to stake LP token", async function () {
    const tx = await this.DHedgeEnv.adapter.connect(this.signers.default).add(FACTORY_REGISTRIES.DHEDGE_TOP);
    await tx.wait();
    ...
    expect(
      (await this.liquidityMigration.staked(holder2Address, this.DHedgeEnv.pool.address)).eq(holder2Balance.div(3)),
    ).to.be.true;
  ...
  it("Should not be able to migrate tokens if the dTop token is not whitelisted in the DHedge Adapter", async function () {
    ...
    const tx = await this.DHedgeEnv.adapter.connect(this.signers.default).remove(FACTORY_REGISTRIES.DHEDGE_TOP);
    await tx.wait();
    // Migrate
    await expect(
      this.liquidityMigration.connect(holder2)["migrate(address,address,address,uint256)"](
  ...
  it("Adding to whitelist from non-manager account should fail", async function () {
    ...
    await expect(this.DHedgeEnv.adapter.connect(this.signers.admin).add(FACTORY_REGISTRIES.DHEDGE_TOP)).to.be.reverted;
  });
  ...
  it("Getting the output token list", async function () {
    ...
    const underlyingTokens = await Promise.all(
      underlyingAssets.map((asset: string) => this.DHedgeEnv.pool.getAssetProxy(asset)),
    );
  ...
  it("Encode withdraw using a non-whitelisted token should fail", async function () {
    ...
    await expect(
      this.DHedgeEnv.adapter.encodeWithdraw(this.DHedgeEnv.pool.address, BigNumber.from(10000)),
    ).to.be.revertedWith("Whitelistable#onlyWhitelisted: not whitelisted lp");
  });

  it("Create strategy", async function () {
    // adding the dHedge Top Token as a whitelisted token
    let tx = await this.DHedgeEnv.adapter.connect(this.signers.default).add(FACTORY_REGISTRIES.DHEDGE_TOP);
    await tx.wait();
  ...
  it("Should migrate tokens to strategy", async function () {
    ...
    const [total] = await estimateTokens(
      this.enso.platform.oracles.ensoOracle,
      this.strategy.address,
      await this.DHedgeEnv.adapter.outputTokens(FACTORY_REGISTRIES.DHEDGE_TOP),
    );
